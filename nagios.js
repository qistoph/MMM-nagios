	...
	getServiceListWithStatus: function(data, state) {
		...
		Object.keys(hostServices).forEach(function(hostname) {
			Object.keys(hostServices[hostname]).forEach(function(servicename) {
				var current_state = hostServices[hostname][servicename].current_state;
				if (current_state == stateId) {
					ret.push({'host': hostname, 'service': servicename, 'state': current_state});
				}
			});
		});
		return ret;
